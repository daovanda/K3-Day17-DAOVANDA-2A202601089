...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Mỗi batch là một statement atomic và UPSERT theo event_id. Vì vậy replay
    không tạo bản sao; payload mới hơn sẽ cập nhật hàng đã tồn tại.
    """
    # Nạp cả batch trong một statement: statement này vừa atomic vừa nhanh
    # hơn hàng trăm câu INSERT riêng lẻ. json_each biến mảng message thành row.
    con.execute(
        f"""
        insert into {TABLE}
        select
